...
class Scheduler(nn.Module):
    ...
    def get_weight(self, batch_support, batch_query, model, pt):
        ...
        for (data_s, data_q) in zip(batch_support, batch_query):
            ...
            torch.cuda.empty_cache()
            # item_emb = model.all_embed[model.n_users: model.n_users + model.n_items, :].detach()
            # best_k = 2000
        ...
        #
        # task_layer_inputs = [torch.stack(input_embedding_norm), torch.stack(input_embedding_cos)]
        #
        # weight = self.forward(task_losses, task_layer_inputs,
        #                       torch.tensor([pt]).long().repeat(len(task_losses)))
